    ...
    public class ExceptionHandlingMiddleware
    {
        ...
        private static async Task HandleExceptionAsync(HttpContext context, Exception exception)
        {
            ...
            if (!context.Response.HasStarted)
            {
                ...
                else if (exception is ProductDomainException prodEx)
                {
                    response.StatusCode = HttpStatusCode.BadRequest;
                    response.Message = prodEx.Message;
                }
                else if (exception is InsufficientStockException insufficientStockEx)
                {
                    response.StatusCode = HttpStatusCode.BadRequest;
                    response.Message = insufficientStockEx.Message;
                    response.Details = $"ProductId: {insufficientStockEx.ProductId}, Requested: {insufficientStockEx.RequestedQuantity}, Available: {insufficientStockEx.AvailableQuantity}";
                }
